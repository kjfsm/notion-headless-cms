...
`nhc doctor`（binding 宣言・webhook secret・token 有効性・同期状態・slug 重複の診断）と `nhc sync`（`schemaModule` の全コレクションをローカルファイルストアへ同期する手動 kick）を CLI コマンドとして配線した。`doctor.ts` の `runDoctorChecks` と `sync-command.ts` の `runSyncCommand` は実装・テストが揃っていたが `nhc` から呼び出せない状態だった。
