    ...
    <div className="relative flex min-h-[calc(100vh-3.5rem)] bg-white">
      ...
      {open && (
        <button
          ...
          className="md:hidden fixed inset-x-0 top-14 bottom-0 z-40 bg-black/30 backdrop-blur-sm"
        />
      )}

      {/* モバイル: スライドインドロワー本体 */}
      <div
        className={`md:hidden fixed top-14 left-0 z-50 h-[calc(100vh-3.5rem)] shadow-xl transition-transform duration-200 ease-out ${
      ...
      <main className="flex-1 min-w-0">
        ...
        <div className="md:hidden sticky top-14 z-30 flex items-center gap-2 border-b border-gray-100 bg-white/90 backdrop-blur px-4 py-2">
          <button
            type="button"
            onClick={() => setOpen((v) => !v)}
            aria-expanded={open}
            aria-label={
              open ? "ドキュメント一覧を閉じる" : "ドキュメント一覧を開く"
            }
        ...
        <div className="mx-auto max-w-3xl px-6 md:px-8 py-10 md:py-12 animate-fade-in-up">
          <Outlet />
        </div>
      </main>
    </div>
