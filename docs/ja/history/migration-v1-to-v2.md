> **このドキュメントは v1→v2 移行時（`@notion-headless-cms/client` 設計）の歴史的記録です。**
> `client`/`core`/`cache`/`notion-source` 等の v2 系パッケージは削除され、
> 現行の唯一のアーキテクチャは `@notion-headless-cms/cms` です。
> 現行の移行ガイドは [`docs/ja/migration/v2-removal.md`](../migration/v2-removal.md) を参照してください。
