...
`createCMS({ render: { imageProxyBase: "/api/cms/images" } })` のように base を変えた場合は、route のパスも合わせて変更する。`@notion-headless-cms/client/next` の `createNextHandler(cms)` を使うと `/api/cms/images/:hash` ルートが自動でマウントされる（個別 route 不要）。
