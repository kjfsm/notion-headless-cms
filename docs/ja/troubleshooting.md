...
- 既定 30 秒は **webhook なしを基準にした保守的な値**です。webhook で更新を即時反映する構成では push が主経路となり、この裏チェックはフォールバックになります。その場合は `swr: { recheckWindowMs: 300_000 }`（5 分）など長めにして Notion API / サブリクエスト消費を抑えてよいです（フォアグラウンドは常にキャッシュ即返しなので体感速度は変わりません）
