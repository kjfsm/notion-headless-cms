...
3. **Build settings** で以下を設定（`hono` 以外はモノリポルートのインストールと nhc generate が必須）:

   | example | Build command | Deploy command |
   |---|---|---|
   | hono / astro / sveltekit | `pnpm --dir=../.. install --frozen-lockfile && pnpm --dir=../.. --filter='@notion-headless-cms/*' build && pnpm generate && pnpm build` | `npx wrangler deploy` |
   | react-router | 同上 | `npx wrangler deploy`（Vite プラグインが `build/server/wrangler.json` を生成し wrangler が読む） |

   `hono` は framework build 不要だが `pnpm build` は no-op スクリプトを叩くだけなので同じコマンドで OK。

4. **Settings > Variables and Secrets**:
   - **Build variables** に `NOTION_TOKEN`（`nhc generate` 用）と `NODE_VERSION=24`（Cloudflare デフォルトは Node 22）
   - **Secrets** に `NOTION_TOKEN`（Worker ランタイム用）
5. main への push で自動ビルド → デプロイ

> Build 時用とランタイム用は別枠なので**両方**に登録する。
> 各 example の `wrangler.toml` には `[build]` を**置いていない**。Dashboard 側の Build command と二重指定になり優先順位の混乱を招くため、ビルドは GitHub App / GitHub Actions のどちらか一系統に統一している。Cloudflare GitHub App を使う場合は Dashboard で必ず Build command を設定する。
> `react-router` example は `@cloudflare/vite-plugin` 経由のビルドで、Vite プラグインが `build/server/wrangler.json` に deploy 設定を書き出し wrangler が読み取る。
