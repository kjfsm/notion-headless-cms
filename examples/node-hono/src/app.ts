  ...
  return html`<!doctype html>
    <html lang="ja">
      ...
      <body>
        ...
        ${raw(body)} ${raw(revalidatorScript())}
      </body>
    </html>`;
}
...
app.get("/ui", (c) => {
  const list = COLLECTION_NAMES.map((col) => `<li><a href="/ui/${col}">${col}</a></li>`).join("");
  return c.html(layout("Notion CMS", `<h1>Notion Headless CMS</h1><ul>${list}</ul>`));
});
...
app.get("/ui/posts/:slug", async (c) => {
  const post = await cms.posts.find(c.req.param("slug"));
  if (!post) return c.html(layout("Not Found", "<h1>404 - 記事が見つかりません</h1>"), 404);
  const content = renderBlocksToHtml(post.blocks, { links: post.links });
  return c.html(layout(post.slug, `<h1>${post.slug}</h1><article>${content}</article>`));
});
