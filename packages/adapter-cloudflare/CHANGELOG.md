...
- 0a938ab: `notionAdapter` が `dbName` オプションを受け付けるようになりました。`dataSourceId` の代わりに `dbName` を渡すと、初回アクセス時に `client.search` で解決されます（結果はインスタンス内にキャッシュ）。
...
- Updated dependencies [bb693f1]
  ...
  - @notion-headless-cms/cache-r2@0.1.3
