...
通常は [`@notion-headless-cms/core`](../core) の `createClient()` 経由で使う。renderer は未指定の場合、core が動的 import でフォールバック注入する。明示的に差し替えたい場合は `createClient({ renderer: renderMarkdown, ... })` で注入する。
