...
- 7423533: `file` ブロックのレンダリングを追加し、`audio` / `pdf` / `image` ブロックのキャプションに `renderRichText` を適用してリンクを含むリッチテキストを正しく出力するようにした。
...
- 8a4a158: embed ブロックを OGP カードではなく iframe で直接表示するように変更

  - provider にマッチしない embed ブロックは OGP を取得せず、直接 `<iframe>` を出力する
  - OGP カードは bookmark ブロック専用の挙動として分離
  - `renderEmbed` から `ogpOptions` 引数を削除

- 3cd6062: Steam・Vimeo・DLsite の組み込み provider を削除し、embed ブロックの汎用フォールバックに OGP カード表示を追加
...
- a653d91: rehype-sanitize-embeds の Biome lint 警告（useLiteralKeys）を修正
