...
export function OgCard({ url, ogp }: OgCardProps) {
  ...
  // Notion 編集者由来の URL を無検証で href/src に載せない(javascript: 等の XSS を防ぐ)。
  const image = safeMediaSrc(ogp?.image);
  ...
  // 危険なスキームの URL はリンク化せず、非リンクのカードとして表示する。
  if (!href) {
    return <div className="block no-underline">{card}</div>;
  }

  return (
    <LinkComp href={href} target="_blank" rel="noopener noreferrer" className="block no-underline">
      {card}
    </LinkComp>
  );
}
