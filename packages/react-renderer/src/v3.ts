...
/**
 * v3 の正規化 block(`NormalizedBlock`)を、既存の react-renderer が期待する
 * `NotionBlock`(Notion API の `BlockObjectResponse` 形状)へ変換する。
 *
 * v3 の同期パイプライン(`normalizeBlock`)は `block[block.type]` のサブオブジェクトを
...
/**
 * v3 の `EntrySnapshot.links`(正規化 pageId → 解決済みリンク)を、
 * 既存の `NotionRenderer` `pageLinks` プロップにそのまま渡せる形へ変換する。
 * `buildPageLinkMap(cms)` の手動呼び出しが不要になる(#437 の摩擦点解消)。
 */
